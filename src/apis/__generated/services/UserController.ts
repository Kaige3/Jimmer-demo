...
export class UserController {
    ...
    async delete(options: UserControllerOptions['delete']): Promise<
        boolean | undefined
    > {
        let _uri = '/user/delete';
        return (await this.executor({uri: _uri, method: 'POST', body: options.body})) as Promise<boolean | undefined>;
    }
    
    async findById(options: UserControllerOptions['findById']): Promise<
    ...
    async query(options: UserControllerOptions['query']): Promise<
    ...
    async register(options: UserControllerOptions['register']): Promise<
        SaTokenInfo
    > {
    ...
    async save(options: UserControllerOptions['save']): Promise<
        string
    > {
        let _uri = '/user/save';
